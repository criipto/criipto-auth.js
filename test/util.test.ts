...
describe("parseQueryParams", () => {
  ...
  test("parses id_token fragment", () => {
    expect(
      parseQueryParams("#id_token=eyJ0eXAiOiJKV1QiLCJhbGci"),
    ).toStrictEqual({
  ...
  test("parses id_token querystring param", () => {
    expect(
      parseQueryParams("?id_token=NHAPY4YFDRYcJ9yTouFkImBWXOR2scUBDeS"),
    ).toStrictEqual({
...
describe("parseAuthorizeParamsFromUrl", function () {
  test("parses url", () => {
    const expected: AuthorizeUrlParamsOptional & {
      domain: string;
      clientID: string;
    } = {
    ...
    expect(
      parseAuthorizeParamsFromUrl(
        "https://mick-gauss-mitid-test.criipto.io/oauth2/authorize?client_id=urn:application:example&redirect_uri=https://jwt.io/&scope=openid&response_type=id_token&response_mode=fragment&nonce=ecnon",
      ),
