...
export class CriiptoQrPromise<T = AuthorizeResponse> extends Promise<T> {
  ...
  constructor(
    executor: (resolve: PromiseResolve<T>, reject: PromiseReject) => void,
  ) {
    let _resolve;
...
export default class CriiptoAuthQrCode {
    ...
    if (!this.#_setupPromise) {
      this.#_setupPromise = Promise.resolve().then(async () => {
        ...
        this.#_websocket = new WebSocket(
          `${config.csdc_wss_url}?clientId=${this.#_clientID}`,
        );
  ...
  #createCanvas(element: HTMLElement) {
    ...
    const existing = element.querySelector(
      '[data-criipto-id="criiptoqrcanvas"]',
    );
  ...
  authorize(
    element: HTMLElement,
    params?: QrAuthorizeParams,
  ): CriiptoQrPromise<AuthorizeResponse> {
    const qrPromise = new CriiptoQrPromise(async (resolve, reject) => {
      ...
      try {
        ...
        if (!config.client.qr_enabled) {
          throw new QrNotEnabledError(
            `QR is not enabled for this Criipto Application. Please go to https://dashboard.criipto.com and enable it.`,
          );
        }

        const qr_intermediary_url =
          config.client.qr_intermediary_url ?? config.qr_intermediary_url;
        ...
        const authorizeUrl = await this.criiptoAuth.buildAuthorizeUrl(
        ...
        const handleMessage = async (message: MessageEvent<any>) => {
          ...
          if (!qrPromise.acknowledged) {
            for (const session of sessionHistory) {
              const decrypted: ArrayBuffer | null =
              ...
              const data: Message = JSON.parse(
                atob(arrayBufferToBase64(decrypted)),
              );
            ...
            // Response phase
            const decrypted: ArrayBuffer | null = await globalThis.crypto.subtle
              .decrypt(
                {
